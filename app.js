...
fetch('https://app-produkt-api-230801161903.azurewebsites.net/products')
  .then((res) => res.json())
...
function renderJson(json1) {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    productsEl.innerHTML += `
        ...
        <p class="card-text text-truncate" style="max-width: 150px;"><b id="title${
          i + 1
        }">${productTitle}</b></p>
...
function renderSubTotal() {
  ...
  subtotalEl.innerHTML = `
        ...
        <div  class="d-flex align-self-flex-end"><p class="m-0"><b><Em>Subtotal(${totalItems} items): ${totalPrice.toFixed(
    0
  )} kr.</em></b></p>
        </div>
